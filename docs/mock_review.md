...
3. Local runtime/sandbox harnesses now include archive-backed public-code fixtures, bounded source-available PyPI package execution, bounded GitHub repo execution, bounded TypeScript repo execution, a bounded public remote endpoint audit, and bounded harmless remote tool calls, but they are still not arbitrary third-party execution environments.
...
4. Authenticated production runtime integration rather than only bounded unauthenticated probes, harmless public tool calls, and toy harnesses.
