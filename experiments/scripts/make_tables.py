...
TYPESCRIPT_REPO_SANDBOX_PATH = RESULTS / "typescript_repo_sandbox.json"
...
def build_table9(third_party: dict) -> tuple[str, str]:
    ...
    pairs = [
        ...
        ("Archive fixtures resolved", str(third_party["archive_fixtures_resolved"])),
...
def build_table12(ts_repos: dict) -> tuple[str, str]:
    """Bounded source-available TypeScript GitHub repo sandbox."""
    title = "Table 12: TypeScript Repo Sandbox"
    headers = ["Metric", "Value"]
    latency = ts_repos["latency_ms"]
    pairs = [
        ("Cases executed", str(ts_repos["cases_executed"])),
        ("Registered tools observed", str(ts_repos["tool_registry_total"])),
        ("CLI delegate calls", str(ts_repos["cli_calls"])),
        ("No unsafe egress", str(ts_repos["acceptance"]["no_unsafe_egress"]).lower()),
        ("Case p95 latency (ms)", f"{latency['p95']:.3f}"),
    ]
    rows_txt = [[k, v] for k, v in pairs]
    rows_tex = [[k.replace("_", "\\_"), v] for k, v in pairs]
    return _txt_table(title, headers, rows_txt), _tex_table(title, "tab:typescript_repo_sandbox", headers, rows_tex, "lr")
...
def main() -> None:
    ...
    typescript_repo_sandbox = load_json(TYPESCRIPT_REPO_SANDBOX_PATH)
    ...
    builders = [build_table1, build_table2, build_table3, build_table4, build_table5, build_table6, build_table7, build_table8, build_table9, build_table10, build_table11, build_table12]
    # Table 1 & 2 need detector, 3 needs ablation, 4 & 5 need redteam, 6 needs generalization, 7 needs runtime harness, 8 needs sandbox harness, 9 needs third-party sandbox, 10 needs corpus-package sandbox, 11 needs GitHub repo sandbox, 12 needs TypeScript repo sandbox.
    args = [detector, detector, ablation, redteam, redteam, generalization, runtime_harness, sandbox_harness, third_party_sandbox, corpus_package_sandbox, github_repo_sandbox, typescript_repo_sandbox]
